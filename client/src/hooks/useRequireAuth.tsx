...
import { type ClaimArray as TClaimArray } from '@idemand/common';
...
export interface UseRequireAuthProps {
  redirectPath?: string;
  returnToPath?: string;
  requiredClaims?: null | TClaimArray; // | CustomClaimKeys[];
...
export const useRequireAuth = ({
  ...
  useEffect(() => {
    ...
    if (!(user && user.uid) && !shouldSignInAnonymously) {
      ...
      navigate(
        redirectPath || createPath({ path: AUTH_ROUTES.CREATE_ACCOUNT }),
        {
          replace: true,
    ...
    if (!allowAnonymous && isAnonymous && !shouldSignInAnonymously) {
      toast.info(
        'You need an login or create an account to access that route.',
      );
    ...
    if (requiredClaims && requiredClaims.length > 0) {
      ...
      let notAuthorized = claims
        ? requiredClaims.every((key) => !claims[key])
        : true;

      if (!!notAuthorized) {
        if (unauthorizedCallback) unauthorizedCallback();

        toast.error(`Missing required permissions to access this route`, {
          duration: 8000,
        });
