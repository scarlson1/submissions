...
function CtxDialog({ children }: { children: ReactNode }) {
  ...
  return (
    <slots.dialog open={Boolean(isOpen)} onClose={handleClose} {...slotProps?.dialog}>
      {children}
    </slots.dialog>
  );
}
...
const Title = () => {
  ...
  return slots.title && title ? <slots.title {...slotProps?.title}>{title}</slots.title> : null;
};
...
const Actions = ({ confirmButtonProps, confirmButtonText = 'submit' }: ActionsProps) => {
  const { onSubmit, handleAccept, handleClose, slots, slotProps, submitDisabled, variant } =
    useDialog();
  ...
  return slots.actions ? (
    <slots.actions {...slotProps.actions}>
      {variant === 'danger' && (
        <>
          {slots.cancelButton && (
            <slots.cancelButton onClick={handleClose} {...slotProps.cancelButton}>
              Cancel
            </slots.cancelButton>
          )}
          {slots.acceptButton && (
