...
import { GridFilterInputValueProps, ValueOptions, useGridRootProps } from '@mui/x-data-grid';
...
export interface GridFilterInputMultipleSingleSelectProps
  extends Omit<
      ...
      | 'getOptionLabel'
    >,
    Pick<GridMultiSelectColDef, 'getOptionLabel' | 'getOptionValue'>,
...
export const GridFilterInputMultipleMultipleSelect = (
  ...
  let resolvedColumn: GridMultiSelectColDef | null = null;
  if (item.field) {
    const column = apiRef?.current?.getColumn(item.field);
    console.log('COLUMN: ', column);
    if (isMultiSelectColDef(column)) {
      resolvedColumn = column;
    }
  }

  const getOptionValue = getOptionValueProp || defGetOptionValue; // || resolvedColumn?.getOptionValue!;
  const getOptionLabel = getOptionLabelProp || defGetOptionLabel; // || resolvedColumn?.getOptionLabel!;

  // const getOptionValue = defGetOptionValue;
  // const getOptionLabel = defGetOptionLabel;
  ...
  // const resolvedValueOptions = React.useMemo(() => ['flood', 'wind'], []);
...
//   }).isRequired,
//   applyValue: PropTypes.func.isRequired /* @typescript-to-proptypes-ignore */,
