...
export interface FloodStateDisclosureProps {
  state: string;
  buttonText?: string;
  textProps?: TypographyProps;
}

export const FloodStateDisclosure: React.FC<FloodStateDisclosureProps> = ({
  state,
  buttonText = 'state disclosure',
  textProps,
}) => {
  const modal = useConfirmation();
  const disclosure = useMemo(() => getHelperText(state), [state]);
