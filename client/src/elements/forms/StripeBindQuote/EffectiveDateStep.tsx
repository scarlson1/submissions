...
export interface EffectiveDateValues {
  effectiveDate: Date;
  effectiveExceptionRequested: boolean;
  effectiveExceptionReason: string | null;
}

export interface EffectiveDateStepProps extends BindQuoteProps<EffectiveDateValues> {
...
EffectiveDateStepProps) => {
  ...
  const handleSubmit = useCallback(async (values: EffectiveDateValues) => {
    try {
      await onStepSubmit({
          ...
          <Box>
            ...
            >
              ...
              <Typography
                variant='body2'
                gutterBottom
                sx={{ pt: 2, pb: 8, color: 'text.secondary' }}
              >
                The date must be at least 15 days from binding policy and within 60 days from the
                initial quote.
              </Typography>
              <FormikDatePicker
                name='effectiveDate'
