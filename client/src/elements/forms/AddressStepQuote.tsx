...
export interface AddressStepTestProps extends Omit<
  FormikAddressProps,
  'setFieldValue'
> {
...
export const AddressStepQuote = ({
  gridProps,
  ...props
}: AddressStepTestProps) => {
  ...
  const activeStatesLayer = new GeoJsonLayer({
    ...
    getFillColor: (f) =>
      activeStates &&
      !!activeStates[f.properties?.SHORT_NAME as keyof ActiveStates]
        ? [0, 125, 255, 50]
        : [255, 255, 255, 20],
