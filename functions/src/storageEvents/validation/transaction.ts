...
export function validateTrxRow(
  data: DeepNullable<Omit<Transaction, 'metadata'>>,
): boolean {
  try {
    switch (data.trxInterfaceType) {
      case 'amendment':
        amendmentTrxValidation(
          data as DeepNullable<Omit<AmendmentTransaction, 'metadata'>>,
        );
        break;
      case 'offset':
        offsetTrxValidation(
          data as DeepNullable<Omit<OffsetTransaction, 'metadata'>>,
        );
        break;
      case 'premium':
        premiumTrxValidation(
          data as DeepNullable<Omit<PremiumTransaction, 'metadata'>>,
        );
        break;
      default:
    ...
    warn(err instanceof Error ? err?.message : 'Validation failed');
    return false;
  }
}
// : asserts data is Omit<BaseTransaction, 'metadata'>
function commonTrxValidation(
  data: DeepNullable<Omit<Transaction, 'metadata'>>,
) {
  verify(
    data.trxType && TransactionType.options.includes(data.trxType),
    'invalid trxType',
  );
  verify(
    data.product && Product.options.includes(data.product),
    'invalid product',
  );
  verify(data.policyId, 'invalid policyId');
  verify(data.externalId, 'invalid locationId');
  verify(data.term, 'term required');
  verify(
    // @ts-expect-error DeepNull type issue
    data.bookingDate && isValid(data.bookingDate.toDate()),
  ...
  verify(
    // @ts-expect-error DeepNull type issue
    data.trxEffDate && isValid(data.trxEffDate?.toDate()),
    'invalid trxEffDate',
  );
  verify(
    // @ts-expect-error DeepNull type issue
    data.trxExpDate && isValid(data.trxExpDate?.toDate()),
    'invalid trxExpDate',
  );
}

function amendmentTrxValidation(
  data: DeepNullable<Omit<AmendmentTransaction, 'metadata'>>,
) {
  commonTrxValidation(data);

  arrayOfStringsVal(
    data.otherInterestedParties,
    'other parties must be an array',
  );
  arrayOfStringsVal(
    data.additionalNamedInsured,
    'additional insureds must be an array',
  );
}
...
function commonOffsetAndPremiumTrxValidation(
  ...
  verify(
    data.termPremium && typeof data.termPremium === 'number',
    'term premium required',
  );
  verify(data.netDWP && typeof data.netDWP === 'number', 'netDWP required');
  verify(
    data.MGACommission && typeof data.MGACommission === 'number',
    'invalid MGA Commission',
  );
  verify(
  ...
  verify(
    data.dailyPremium && typeof data.dailyPremium === 'number',
    'invalid dailyPremium',
  );
  verify(
    !data.netErrorAdj || typeof data.netErrorAdj === 'number',
    'invalid netErrorAdj',
  );

  verify(typeof data.surplusLinesTax === 'number', 'invalid surplusLinesTax');
  verify(
    typeof data.surplusLinesRegulatoryFee === 'number',
    'invalid SL Regulatory Fee',
  );
