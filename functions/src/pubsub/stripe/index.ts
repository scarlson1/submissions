...
export const createtaxtransactions = onMessagePublished<ChargeSucceededPayload>(
  ...
  async (event) => {
    await (await import('./createTaxTransactionsOnChargeSucceeded.js')).default(event);
  }
);

export const createtransfers = onMessagePublished<ChargeSucceededPayload>(
  { topic: PaymentPubSubTopics.enum.CHARGE_SUCCEEDED, secrets: [stripeSecretKey] },
  async (event) => {
    await (await import('./createTransfersOnChargeSucceeded.js')).default(event);
  }
);
