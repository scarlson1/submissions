import { UserAccess, getUserAccessRef } from '@idemand/common';
...
export default async (
  ...
  try {
    ...
    if (accessData.userId)
      visibleBy.push(
        ...getVisibleBy({ userId: accessData.userId, agentId: null, orgId: null }, ['user'])
      );
    for (let agentId of accessData?.agentIds) {
      visibleBy.push(...getVisibleBy({ agentId, orgId: null, userId: null }, ['agent']));
    }
    for (let orgId of accessData.orgIds) {
      visibleBy.push(...getVisibleBy({ orgId, agentId: null, userId: null }, ['orgAdmin']));
    }
    ...
    index.partialUpdateObject(updates, { createIfNotExists: false }).then(({ objectID }) => {
      info(`updated algolia user record's "visibleBy" property (${objectID})`);
    });
  } catch (err: any) {
    reportErr(`ERROR UPDATING USER IN ALGOLIA INDEX (${userId})`, { params: event.params }, err);
