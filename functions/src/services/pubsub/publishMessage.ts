...
function getPubSubClient() {
  ...
  const isFunctionsEmulator =
    process.env.FUNCTIONS_EMULATOR === 'true' || Boolean(process.env.EVENTARC_EMULATOR);
  const configuredHost = process.env.PUBSUB_EMULATOR_HOST || pubSubEmulatorHost.value();
...
export async function publishMessage(topicNameOrId: string, data: { [key: string]: any }) {
  ...
  try {
    ...
    const messageId = await client.topic(topicNameOrId).publishMessage({ data: dataBuffer });
