...
app.post('/webhook', async (req: StripeWebhookRequest, res: Response) => {
  ...
  if (endpointSecret) {
    ...
    try {
      if (!req.rawBody) {
        reportErr(
          'Missing raw request body for Stripe webhook signature verification',
        );
        res.status(400).send({});
        return;
      }
    ...
    event = JSON.parse(req.body) as StripeWebhookEvent;
  } else {
