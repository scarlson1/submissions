...
// token ? token[Claim.enum.iDemandAdmin] || false : false;

export const isAgent = (token: DecodedIdToken | undefined) =>
  hasClaim(token, 'agent'); // token ? token[Claim.enum.agent] || false : false;

export const isOrgAdmin = (token: DecodedIdToken | undefined) =>
  hasClaim(token, 'orgAdmin');
// token ? token[Claim.enum.orgAdmin] || false : false;
