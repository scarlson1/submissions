...
export const generateRequestToken = () => {
  return new Promise<string>(async (resolve, reject) => {
    let userApiKey: string | undefined = await getSecret('SPATIALKEY_USER_API_KEY');
    let orgApiKey: string | undefined = await getSecret('SPATIALKEY_ORG_API_KEY');
    let orgSecretKey: string | undefined = await getSecret('SPATIALKEY_ORG_SECRET_KEY');
