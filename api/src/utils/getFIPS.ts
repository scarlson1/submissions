...
export const getByCountyAndState = (county: string, state: string) => {
  ...
  if (!state || !county) throw new Error('You must provide a state abbreviation and county name.');
  if (state.length !== 2) throw new Error('State must be a two letter state abbreviation.');
  if (typeof state !== 'string') throw new Error('State abbreviation must be a string.');
  if (typeof county !== 'string') throw new Error('County name must be a string.');
...
export const getByFipsAndState = (fips: string, state: string) => {
  ...
  const match = FIPS.find(
    (county) => county.countyFP === fips && county.state === state.toUpperCase()
  );
